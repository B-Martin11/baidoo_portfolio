...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.warn('Backend API offline, loading static JSON fallback...', error);
      try {
        const fallbackRes = await fetch('/db_content.json');
        if (!fallbackRes.ok) throw new Error('Static fallback file not OK');
        const fallbackData = await fallbackRes.json();
        setPortfolioData(fallbackData);
      } catch (fallbackError) {
        console.error('Failed to load static fallback data as well:', fallbackError);
      }
    }
  };
